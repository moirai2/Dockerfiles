...
sub help{
	...
	if(defined($opt_H)){
		...
		print "############################## Updates ##############################\n";
		print "\n";
		print "2022/01/41  Link with openstack\n";
...
sub test{
	...
	close(OUT);
	testCommand("perl $prgdir/moirai2.pl -d test/moirai -i '\$input' -o '\$output' build < test/1.sh|xargs cat","{\"https://moirai2.github.io/schema/daemon/bash\":[\"ls \$input > \$output\"],\"https://moirai2.github.io/schema/daemon/input\":[\"\$input\"],\"https://moirai2.github.io/schema/daemon/output\":[\"\$output\"]}");
	testCommand("perl $prgdir/moirai2.pl -d test/moirai -i 'root->directory->\$input' -o 'root->content->\$output' build < test/1.sh|xargs cat","{\"https://moirai2.github.io/schema/daemon/bash\":[\"ls \$input > \$output\"],\"https://moirai2.github.io/schema/daemon/input\":[\"\$input\"],\"https://moirai2.github.io/schema/daemon/output\":[\"\$output\"]}");
	unlink("test/1.sh");
	#testing ls
	mkdir("test/dir");
