...
//############################## insert.php ##############################
	...
	if($file==null)exit(1);
	system("rdf.pl -q -d $db -f json insert < $file");
	unlink($file);
//############################## delete.php ##############################
	...
	if($json==null)exit(1);
	system("rdf.pl -d $db -f json delete < $json");
	unlink($json);
	//############################## update.php ##############################
	...
	$json=tmpData($_POST["data"]);
	if($json==null)exit(1);
	system("rdf.pl -d $db -f json update < $json");
	unlink($json);
//############################## select.php ##############################
	...
	if($json==null)exit(1);
	exec("rdf.pl -d $db -f json select < $json",$array);
	unlink($json);
...
//############################## progress.php ##############################
	...
	$db=filterDatabasePath($_POST["rdfdb"]);
	if($db==null)exit(1);
	exec("rdf.pl -d $db progress",$array);
	foreach($array as $line){echo "$line\n";}
//############################## query.php ##############################
	...
	$json=tmpData($_POST["query"]);
	if($json==null)exit(1);
	exec("rdf.pl -d $db -f json query < $json",$array);
	unlink($json);
	foreach($array as $line){echo "$line\n";}
//############################## symlink.php ##############################
